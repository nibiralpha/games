...
const getMonthlyGames = async (fromDate: string, toDate: string): Promise<TrendingGameResponse> => {
  const response = await axios.get<TrendingGameResponse>(`${BASEURL}/api/recent-games?start=${fromDate}&end=${toDate}`);

  return response.data;
};

const getGamesLastRecentAnicipeted = async (): Promise<ReleaseCalendarResponse> => {
  const response = await axios.get<ReleaseCalendarResponse>(`${BASEURL}/api/release-calendar`);
